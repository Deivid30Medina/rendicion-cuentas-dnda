  ...
  return (
    <nav
      className={`w-full bg-color-dnda flex items-center justify-between z-50 custom-max-h 
        ${isAtTop ? 'h-40 flex-col pt-3 transition-all duration-1000' : 'fixed top-0 left-0 flex-row min-h-16 transition-all duration-1000 px-8'}`}
    >
      ...
      <ul className={`absolute w-full bg-color-dnda flex flex-col items-center justify-center z-50 top-16 left-0 transition-all duration-700 ease-in-out overflow-hidden gap-12 ${isMenuOpen ? (isAtTop ? 'max-h-[550px] mt-12 opacity-100' : 'mt-0 opacity-100') : 'max-h-0 opacity-0'} xl:flex xl:flex-row xl:static xl:top-0 xl:bg-transparent xl:w-auto xl:max-h-none xl:opacity-100`}>
